			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("cost_center") === oObject.cost_center;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].cost_center;
									} else {
										sCurrentId = aItemsNotInTheList[0].cost_center;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("cost_centerhelpSet", {
										cost_center : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("cost_center"),
						title: oBindingContext.getProperty("cost_center")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<cost_centerhelpSet> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("cost_center");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all cost_centerhelpSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<cost_centerhelpSet> (" + iExpectedLength + ")"}),
								success : function () {
